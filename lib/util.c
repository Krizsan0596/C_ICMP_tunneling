...
ssize_t receive_packet(int socket, uint8_t *buffer, size_t buffer_size) {
    ...
    if (bytes_received < 0) {
        perror(stderr, "Receiving failed.\n");
        return -EIO;
    }
...
icmp_packet* generate_custom_ping_packet(uint16_t id, uint16_t sequence, uint8_t ttl, const uint8_t *payload, size_t payload_len, size_t *packet_size) {
    if (payload == NULL) {
        perror(stderr, "generate_custom_ping_packet: payload argument must not be NULL.\n");
        return NULL;
    }
    
    if (payload_len > PAYLOAD_SIZE) {
        perror(stderr, "generate_custom_ping_packet: payload length exceeds maximum of %d bytes.\n", PAYLOAD_SIZE);
        return NULL;
    }
    ...
    if (packet == NULL) {
        perror(stderr, "generate_custom_ping_packet: failed to allocate memory.\n");
        return NULL;
    }
...
int64_t send_packet(int socket, const char *dest_ip, icmp_packet *packet, size_t packet_size, sliding_window *window, bool resend, bool ack) {
    ...
    if (inet_pton(AF_INET, packet->dest_ip, &dest_addr.sin_addr) <= 0) {
        perror(stderr, "Invalid destination IP address.\n");
    ...
    if (packet->ttl > 0) {
        if (setsockopt(socket, IPPROTO_IP, IP_TTL, &packet->ttl, sizeof(packet->ttl)) < 0) {
            perror(stderr, "Failed to set ttl value.\n");
            if (default_packet) free(default_packet);
            return -EINVAL;
        }
    }
    ...
    if (bytes_sent < 0) {
        perror(stderr, "Failed to send packet.\n");
        if (default_packet) free(default_packet);
        return -EIO;
    }

    if ((size_t)bytes_sent != packet_size) {
        perror(stderr, "Partial send.\n");
        if (default_packet) free(default_packet);
        return -EIO;
    }
    ...
    if (!ack) {
        ...
        if (!resend) {
            ...
            if (window->count >= WINDOW_SIZE) {
                ...
                perror(stderr, "Window is full. Cannot track new packet.\n");
                if (default_packet) free(default_packet);
                return -EBUSY;
            }
